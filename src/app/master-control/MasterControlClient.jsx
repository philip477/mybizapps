...
function AppIcon({ icon, name }) {
  ...
  if (icon) return <span style={{ fontSize: 30, lineHeight: 1 }}>{icon}</span>
  return <span style={{ color: '#fff', fontSize: 18, fontWeight: 600 }}>{name?.charAt(0).toUpperCase() || '?'}</span>
}

export default function MasterControlClient({ apps = [] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <PageHeader title="Master Control" onBack={() => router.push('/')} />

      <div style={{ flex: 1, overflowY: 'auto' }}>
        {apps.length === 0 ? (
          <div style={{ padding: '48px 16px', textAlign: 'center', color: '#5580a0', fontSize: 14 }}>
            No master control apps have been configured yet.
          </div>
        ) : (
          apps.map((app) => (
            <div
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: 6,
                  flexShrink: 0,
                  background: app.app_icon ? 'transparent' : '#1a56a0',
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                <AppIcon icon={app.app_icon} name={app.app_name} />
              </div>
